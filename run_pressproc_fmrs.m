...
% 
% USAGE:
% [out_stimOFF,out_stimON,out_w]=run_pressproc_fmrs(filestring,blockDesign,leadingAvgsToRmv);
% 
% DESCRIPTION:
% Processing script for functional MRS data acquired using the PRESS MRS 
...
function [out_stimOFF,out_stimON,out_w]=run_pressproc_fmrs(filestring,blockDesign,leadingAvgsToRmv);
...
nBadAvgTotal=0;
nbadAverages=1;
allAveragesLeft=[1:out_cc.sz(out_cc.dims.averages)]';  % jay edit
allBadAverages=[];                                       % jay edit
    ...
    while sat=='n'||sat=='N'
       ...
       allBadAverages=[];                                     % jay edit
        ...
        while nbadAverages>0;
            ...
            nbadAverages=length(badAverages); % jay edit
            ...
            pause
            close all;
%               nbadAverages=length(badAverages);         %added by jay
%             nBadAvgTotal=nBadAvgTotal+nbadAverages
%             out_cc2=out_rm;
%             iter=iter+1;
%             pause
%             close all;                                  % until here
        end
        figure('position',[0 50 560 420]);
        subplot(1,2,1);
...
%follows:
if blockDesign(1)<0
    blockDesignTrimmedON=blockDesign-leadingAvgsToRmv;
    if isrow(blockDesign)
        blockDesignTrimmedOFF=blockDesign+[0 (leadingAvgsToRmv)*ones(1,length(blockDesign)-1)];
    elseif iscolumn(blockDesign)
        blockDesignTrimmedOFF=blockDesign+[0;(leadingAvgsToRmv)*ones(length(blockDesign)-1,1)];
    end
...
elseif blockDesign(1)>0
    blockDesignTrimmedOFF=blockDesign-leadingAvgsToRmv;
    if isrow(blockDesign)
        blockDesignTrimmedON=blockDesign+[0 (leadingAvgsToRmv)*ones(1,length(blockDesign)-1)];
    elseif iscolumn(blockDesign)
        blockDesignTrimmedON=blockDesign+[0;(leadingAvgsToRmv)*ones(length(blockDesign)-1,1)];
    end
    for n=1:length(blockDesignTrimmedON);
